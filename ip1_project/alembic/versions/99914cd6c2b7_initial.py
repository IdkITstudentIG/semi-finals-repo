...
Revision ID: 99914cd6c2b7
Revises: 
Create Date: 2026-04-01 01:39:29.111277
...
def upgrade() -> None:
    ...
    op.create_table('ebdao_cashiers',
    ...
    op.create_table('ebdao_discounts',
    ...
    op.create_table('ebdao_suppliers',
    ...
    sa.Column('status', sa.String(length=20), nullable=False),
    sa.ForeignKeyConstraint(['category_id'], ['ebdao_categories.category_id'], ),
    sa.ForeignKeyConstraint(['supplier_id'], ['ebdao_suppliers.supplier_id'], ),
    sa.PrimaryKeyConstraint('product_id'),
    sa.UniqueConstraint('sku')
    )
    op.create_table('ebdao_purchase_orders',
    ...
    sa.Column('status', sa.String(length=20), nullable=False),
    sa.ForeignKeyConstraint(['cashier_id'], ['ebdao_cashiers.cashier_id'], ),
    sa.ForeignKeyConstraint(['supplier_id'], ['ebdao_suppliers.supplier_id'], ),
    ...
    sa.Column('status', sa.String(length=20), nullable=False),
    sa.ForeignKeyConstraint(['cashier_id'], ['ebdao_cashiers.cashier_id'], ),
    sa.ForeignKeyConstraint(['customer_id'], ['ebdao_customers.customer_id'], ),
    sa.PrimaryKeyConstraint('sale_id')
    )
    op.create_table('ebdao_po_items',
    ...
    sa.ForeignKeyConstraint(['po_id'], ['ebdao_purchase_orders.po_id'], ),
    sa.ForeignKeyConstraint(['product_id'], ['ebdao_products.product_id'], ),
    sa.PrimaryKeyConstraint('po_item_id')
    )
    op.create_table('ebdao_sale_items',
    ...
    sa.ForeignKeyConstraint(['product_id'], ['ebdao_products.product_id'], ),
    sa.ForeignKeyConstraint(['sale_id'], ['ebdao_sales.sale_id'], ),
    sa.PrimaryKeyConstraint('sale_item_id')
    )
    op.create_table('ebdao_stock_movements',
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('ebdao_stock_movements')
    op.drop_table('ebdao_sale_items')
    op.drop_table('ebdao_po_items')
    op.drop_table('ebdao_sales')
    op.drop_table('ebdao_purchase_orders')
    op.drop_table('ebdao_products')
    op.drop_table('ebdao_suppliers')
    op.drop_table('ebdao_discounts')
    op.drop_table('ebdao_customers')
    op.drop_table('ebdao_categories')
    op.drop_table('ebdao_cashiers')
    # ### end Alembic commands ###
